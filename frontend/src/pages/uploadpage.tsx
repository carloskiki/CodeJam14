...
const [plusCode, setPlusCode] = useState('')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const thumbnailUrl = thumbnail
        ? await uploadImage(
            thumbnail,
            `${newIndex}/thumbnail/${thumbnail.name}`
          )
        : imageUrls[0];
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <Card className="p-6">
        {/* Container for Back to Listings Button */}
        <div className="flex justify-start">
          <Link
            to="/"
            className="text-black font-bold hover:text-darkgray-200 inline-flex items-center gap-2 px-4 py-2 border border-black rounded-md hover:bg-gray-100 transition-colors duration-200"
          >
